...
from utils_mix import rand_bbox, one_hot, interleave_offsets, interleave
import numpy as np
#from pruner import print_sparsity_statistics
...
parser.add_argument('--decay-epochs', type=int,  nargs='+',  default=None, metavar='N',
...
# Misc
parser.add_argument('--seed', type=int, default=66, metavar='S',
...
parser.add_argument('--softmax-multiplier',default = 1.0, type=float, help='Sharpening/smoothing the softmax, which can be used together with mixup.')
parser.add_argument("--local_rank", default=0, type=int)

parser.add_argument('--reset-lr-scheduler', default=None, type=float, help='Reset the learning rate schedular to the specified lr')
parser.add_argument('--alpha', default=None, type=float, help='Hard alpha-binary threshold for Ternary')
parser.add_argument('--cycle-limit', default=None, type=int, help='Number of cycles for cosine scheduling')
parser.add_argument('--freeze-binary', action='store_true', default=False, help='Freeze the binary convolution in regular model, in order to stablize SWA.')
parser.add_argument('--clean-train', action='store_true', default=False, help='Using clean training dataset instead of augmented dataset.')
parser.add_argument('--prune', action='store_true', default=False, help='Using pruner to get sparse weights for the SE layers.')
parser.add_argument('--cutmix_prob', type=float, default=0.0, help='cutmix_probability (default: 0.)')
...
def _parse_args():
    ...
    # if only one elements presented in args.decay_epochs, it is the uniform interval between the decay
    if isinstance(args.decay_epochs, list) and len(args.decay_epochs)==1:
        args.decay_epochs = int(args.decay_epochs[0])
    if args.cutmix_prob > 0 and not args.no_prefetcher:
        raise Exception('Feature-level constraints can only suppport single GPU for now')
...
def forward_binarizable_wraper(self):
    def forward_binarizable(x):
        return (F.conv2d(x, self._get_weight('weight'), self.bias, self.stride, self.padding, self.dilation, self.groups))
    return(forward_binarizable)

def _get_weight_binarizable_wraper(self):
    def _get_weight_binarizable(name):
        w = getattr(self, name)
        return ForwardSign.apply(w)
    return(_get_weight_binarizable)

def Conv2d_binary_patch(m):
    m._get_weight = _get_weight_binarizable_wraper(m)
    m.forward = forward_binarizable_wraper(m)


# Ternarize/Binarize the weights
def Model_binary_patch(model):
    ...
    def disable_BatchNorm_affine(bn):
        ...
        bn.weight = None
        bn.bias = None
    print('Modifying binarizable network.')
    for k, m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            if check_contain(k, ['conv_stem', 'se.conv_reduce','se.conv_expand']): # does not convert the first conv layer and squeeze-excite
                continue
            elif m.kernel_size == (1,1): #  only the non-squeeze-excite conv with 1x1 kernel will be ternarized/binarized
                Conv2d_binary_patch(m)

def Model_freeze_binary(model):
    def check_contain(k,L):
        res = [l in k for l in L]
        return(any(res))
    print('Freezing binary weights.')
    for k, m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            if check_contain(k, ['conv_stem', 'se.conv_reduce','se.conv_expand']): # does not convert the first conv layer and squeeze-excite
                continue
            elif m.kernel_size == (1,1): #  only the non-squeeze-excite conv with 1x1 kernel are ternarized/binarized
...
def loss_consistency_without_prefetcher(model, loss_fn, consistency_criterion, input, target, args):

    input, target = input.cuda(), target.cuda()
    r = np.random.rand(1)
    if args.mixup > 0 and r < args.cutmix_prob:

        # mixup (bs64: 80.8), cutmix (bs64: 80.83), cutmix + consistency (bs64: 81.12, f.t. 81.81)
        ...
        target_a_var = one_hot(torch.autograd.Variable(target_a), args.num_classes)
        target_b_var = one_hot(torch.autograd.Variable(target_b), args.num_classes)
        inputs = torch.cat([input_var, input_a, input_b], dim=0)
        inputs = list(torch.split(inputs, args.batch_size))
        inputs = interleave(inputs, args.batch_size)
        outputs = [model(inputs[0])]
        for inp in inputs[1:]:
            outputs.append(model(inp))
        outputs = interleave(outputs, args.batch_size)
        output = outputs[0]
        output_2 = lam * outputs[1] + (1 - lam) * outputs[2]
        consistency_loss = consistency_criterion(output, output_2)
        loss = loss_fn(output, target_a_var) * lam + loss_fn(output, target_b_var) * (
                1. - lam) + consistency_loss
    else:
        # compute output
        input_var = input
        target_var = one_hot(target, args.num_classes)
        # target_var = target
...
def train_epoch(
        ...
        lr_scheduler=None, saver=None, output_dir='', use_amp=False, tensorboard_writer=None, pruner=None):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.prefetcher:
            if args.cutmix_prob > 0:
                # feature-level constraints can only suppport single GPU for now
                loss = loss_consistency_without_prefetcher(model, loss_fn, consistency_criterion, input, target, args)
            else:
                loss = loss_mixup_without_prefetcher(model, loss_fn, input, target, epoch, args)
        else:
            output = model(input) # mixup is handled in prefetcher
            loss = loss_fn(output, target)
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                ...
                if tensorboard_writer is not None:
                    tensorboard_writer.add_scalar('Train/loss', losses_m.avg, epoch)
        ...
        if saver is not None and args.recovery_interval and (
            ...
            saver.save_recovery(
                model, optimizer, args, epoch, use_amp=use_amp, batch_idx=batch_idx)
...
def validate(model, loader, loss_fn, args, log_suffix='', tensorboard_writer=None, epoch=None):
    ...
    if tensorboard_writer is not None:
        tensorboard_writer.add_scalar('Test/loss', losses_m.avg, epoch)
        tensorboard_writer.add_scalar('Test/acc_1', prec1_m.avg, epoch)
        tensorboard_writer.add_scalar('Test/acc_5', prec5_m.avg, epoch)
        tensorboard_writer.add_scalar('Test/alpha', alpha, epoch)
